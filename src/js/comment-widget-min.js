...
function getComments(){c_submitButton.disabled;c_replyingText.style.display='none';c_replyInput.value='';if(s_commentsOpen){document.getElementById(`entry.${s_nameId}`).value='';document.getElementById(`entry.${s_websiteId}`).value='';document.getElementById(`entry.${s_textId}`).value='';document.getElementById(`answer.${s_textId}`).value=''}
...
v_amountOfPages=Math.ceil(comments.length/s_commentsPerPage);v_commentMax=s_commentsPerPage*v_pageNum;v_commentMin=v_commentMax-s_commentsPerPage;comments.reverse();for(i=0;i<comments.length;i++){let comment=createComment(comments[i]);let button=document.createElement('button');button.innerHTML=s_replyButtonText;button.value=comment.id;button.setAttribute('onclick',`openReply(this.value)`);button.className='c-replyButton';comment.appendChild(button);comment.style.display='none';if(i>=v_commentMin&&i<v_commentMax){comment.style.display='block'}
